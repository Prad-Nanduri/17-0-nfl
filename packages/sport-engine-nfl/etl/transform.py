...
def _players(
    ...
    for player_id in sorted(by_id):
        ...
        if (
            str(_field(row, "position")).strip().upper() == "LB"
            and str(_field(row, "depth_chart_position")).strip().upper() == "OLB"
        ):
            group = "DL"
        if group is None:
...
def _stats_row(
    ...
    stats: dict[str, Any] = {}
    games = int(weekly["week"].nunique()) if not weekly.empty and "week" in weekly else 0
    stats["offenseSnaps"] = _sum(snap, "offense_snaps") if snap is not None else None
    stats["defenseSnaps"] = _sum(snap, "defense_snaps") if snap is not None else None
    stats["stSnaps"] = _sum(snap, "st_snaps") if snap is not None else None
    stats["games"] = games
    ...
    defense = defensive[(defensive["player_id"].eq(player_id)) & (defensive["team"].eq(team)) & defensive["season_type"].eq("REG")] if "player_id" in defensive and "team" in defensive else defensive.iloc[0:0]
    kick = kicking[(kicking["player_id"].eq(player_id)) & (kicking["team"].eq(team)) & kicking["season_type"].eq("REG")] if "player_id" in kicking and "team" in kicking else kicking.iloc[0:0]
    ...
    pfr_row = pfr.get(str(pfr_id)) if pfr_id else None
    ...
    pass_team = pfr_pass[pfr_pass["team"].eq(team)] if "team" in pfr_pass else pfr_pass.iloc[0:0]
    rush_team = pfr_rush[pfr_rush["team"].eq(team)] if "team" in pfr_rush else pfr_rush.iloc[0:0]
    ...
    stats["penalties"] = int((pbp["penalty_player_id"].eq(player_id) & pbp["season_type"].eq("REG")).sum()) if pbp is not None else None
    stats["punts"] = None
    stats["grossAvg"] = None
    stats["netAvg"] = None
    stats["inside20Rate"] = None
    stats["clutchNetAvg"] = None
    if pbp is not None:
        punts = pbp[pbp["punter_player_id"].eq(player_id) & pbp["season_type"].eq("REG") & pbp["punt_attempt"].eq(1)]
        stats["punts"] = int(len(punts))
        stats["grossAvg"] = number(pd.to_numeric(punts["kick_distance"], errors="coerce").mean()) if not punts.empty else None
        net = pd.to_numeric(punts["kick_distance"], errors="coerce") - pd.to_numeric(punts["return_yards"], errors="coerce").fillna(0) - 20 * pd.to_numeric(punts["touchback"], errors="coerce").fillna(0)
        stats["netAvg"] = number(net.mean()) if not punts.empty else None
        stats["inside20Rate"] = number(pd.to_numeric(punts["punt_inside_twenty"], errors="coerce").mean()) if not punts.empty else None
        clutch = punts[pd.to_numeric(punts["qtr"], errors="coerce") >= 4]
        clutch_net = pd.to_numeric(clutch["kick_distance"], errors="coerce") - pd.to_numeric(clutch["return_yards"], errors="coerce").fillna(0) - 20 * pd.to_numeric(clutch["touchback"], errors="coerce").fillna(0)
        stats["clutchNetAvg"] = number(clutch_net.mean()) if len(clutch) >= 3 else None
        kickoffs = pbp[pbp["kicker_player_id"].eq(player_id) & pbp["season_type"].eq("REG") & pbp["kickoff_attempt"].eq(1)]
        stats["kickoffTouchbackRate"] = number(pd.to_numeric(kickoffs["touchback"], errors="coerce").mean()) if not kickoffs.empty else None
        fgs = pbp[pbp["kicker_player_id"].eq(player_id) & pbp["season_type"].eq("REG") & pbp["field_goal_attempt"].eq(1)]
        clutch_fgs = fgs[pd.to_numeric(fgs["qtr"], errors="coerce") >= 4]
        made = clutch_fgs["field_goal_result"].eq("made").sum()
    ...
    stats["brokenTackleRate"] = _ratio(number(_field(pfr.get(str(pfr_id)), "brk_tkl")) if pfr_id and str(pfr_id) in pfr else None, stats["carries"])
    ...
    stats["pressureRate"] = _ratio(stats["pfrPressures"] or ((stats["qbHits"] or 0) + (stats["defSacks"] or 0)), stats["defenseSnaps"])
    ...
    stats = {key: json_value(value) for key, value in stats.items()}
    return {"gsisId": player_id, "franchiseKey": team, "season": int(player.get("season", 2023)), "position": player["primaryPosition"], "positionGroup": group, "eraTier": "full_feature", "games": games, "stats": stats, "isTeamLevelProxy": group == "OL"}
...
def transform(data: dict[str, pd.DataFrame | None], season: int, out: Path) -> dict[str, int]:
    ...
    if seasonal is not None:
        ...
        for _, seasonal_row in seasonal.head(5).iterrows():
            ...
            for weekly_key, seasonal_key in [
                ...
                if weekly_total is not None and seasonal_total is not None and abs(weekly_total - seasonal_total) > 0.01:
                    print(
                        f"weekly cross-check diff player={player_id} {weekly_key}: "
                        f"weekly={weekly_total} seasonal={seasonal_total}"
                    )
    ...
    pfr = _pfr_map(pfr_def)
    pfr.update(_pfr_map(pfr_rec))
    pfr.update(_pfr_map(pfr_rush))
    ...
    for player in players:
        player_teams = sorted({team for (player_id, team) in weekly_groups if player_id == player["gsisId"]})
        if not player_teams:
        ...
        for team in player_teams:
            ...
            stats_rows.append(_stats_row(player, team, weekly_frame, snap_frame, pfr, pfr_pass, pfr_rush, pfr_def, defensive, kicking, pbp, weekly))
    legacy_rows = []
    ...
    if draft_picks is not None:
        for _, row in draft_picks.iterrows():
            ...
            legacy_rows.append({
                "pfrId": pfr_id,
                "gsisId": json_value(_field(row, "gsis_id")),
                ...
                "eraTier": "legacy",
                "stats": {key: json_value(_field(row, source)) for key, source in {
                    "carAv": "car_av", "weightedAv": "w_av", "draftTeamAv": "dr_av", "proBowls": "probowls", "allPro": "allpro", "seasonsStarted": "seasons_started", "games": "games", "passCompletions": "pass_completions", "passAttempts": "pass_attempts", "passYards": "pass_yards", "passTds": "pass_tds", "passInts": "pass_ints", "rushAttempts": "rush_atts", "rushYards": "rush_yards", "rushTds": "rush_tds", "receptions": "receptions", "recYards": "rec_yards", "recTds": "rec_tds", "defSoloTackles": "def_solo_tackles", "defInts": "def_ints", "defSacks": "def_sacks"}.items()},
            })
